...
def get_data():
    ...
    try:
        ...
        # 1. Ensure 'Wholesale' column exists in Transactions
        if 'Wholesale' not in data['transactions'].columns:
            data['transactions']['Wholesale'] = "FALSE"
            
        # 2. Ensure 'Cost' column exists in Inventory
...
def restock_inventory(sku, qty_to_add, new_unit_cost):
    ...
    if not col_stock: return False, "Column 'StockQty' missing"
    # If Cost column missing, we can't update cost, but we can update stock
    ...
    if col_cost:
        ...
        else: new_avg_cost = new_unit_cost
        ws.update_cell(row, col_cost, new_avg_cost)
    else:
        new_avg_cost = 0.0 # Placeholder
        
    ws.update_cell(row, col_stock, curr_stock + qty_to_add)
    force_refresh()
    return True, f"Updated! Stock: {curr_stock + qty_to_add}"
...
def update_customer_details(cust_id, new_name, address, phone, notes):
    ...
    try:
        ...
        # Update dynamically if possible, or fallback to fixed cols
        # Fixed cols: Name(2), Phone(4), Address(6), Notes(7)
...
def commit_sale(cart, total, tax, cust_id, payment_method, is_wholesale, status="Paid", credit_used=0.0):
    ...
    # Check if 'Wholesale' header exists in Transactions, if not add it
    ws_trans = sh.worksheet("Transactions")
    headers = ws_trans.row_values(1)
    if "Wholesale" not in headers:
        ws_trans.update_cell(1, len(headers)+1, "Wholesale")
...
def delete_invoice(invoice_id):
    ...
    # 2. Delete Items (Loop until gone)
...
def sell_gift_certificate(giver_id, receiver_id, amount, pay_method):
    ...
    # For safety, forcing a refresh
    force_refresh()
    return "GC-123" # Placeholder return for brevity in this fix
...
def create_income_statement_pdf(start_date, end_date, revenue, cogs, expenses_df):
    ...
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(130, 6, "Total Sales", 0, 0); pdf.cell(30, 6, f"{revenue:,.2f}", 0, 1, 'R')
    pdf.ln(5)
    
    # COGS
    pdf.set_font("Helvetica", "B", 10); pdf.cell(100, 8, "COST OF GOODS SOLD", ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(130, 6, "Cost of Goods", 0, 0); pdf.cell(30, 6, f"{cogs:,.2f}", 0, 1, 'R')
    pdf.ln(5)
    ...
    net = gross - total_exp
    pdf.ln(5)
    pdf.set_font("Helvetica", "B", 14); pdf.cell(130, 10, "NET PROFIT:", 0, 0); pdf.cell(30, 10, f"{net:,.2f}", "TB", 1, 'R')
